...
def mqtt_sent(pm_2_5, pm_10, aqi_2_5, aqi_10, temp):
    ...
    mqtt_playload = 'field1=' + str(pm_2_5) + '&field2=' + str(
        aqi_2_5) + '&field3=' + str(pm_10) + '&field4=' + str(aqi_10) + '&field5=' + temp
    publish.single(mqtt_urn, payload=mqtt_playload, hostname=mqtt_hostname,
                   port=mqtt_port, tls=mqtt_tsl, transport=mqtt_transport)
...
try:
    ...
    print('[INFO]', datetime.now(), pm_2_5,
          pm_10, aqi_2_5, aqi_10, temp, sep=',')
except TypeError:
    print('[FATAL]', datetime.now(), "Can't get data.", sep=',')
except:
    print('[ERROR]', datetime.now(), pm_2_5,
          pm_10, aqi_2_5, aqi_10, temp, sep=',')
